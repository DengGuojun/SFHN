...
public class TrainingClassInfoCountBusiness {
	public List<String> getTrainingClassInfoCountList(TrainingOrganizationInfoBean trainingOrganizationInfoBean,
			String level) {
		// 统计不同类型的开班数
		...
		trainingClassCondMap.put("classStatus", TrainingClassInfoConfig.CLASS_STATUS_OPENED);
		...
		if (!trainingClassInfoList.isEmpty()) {
			for (TrainingClassInfoBean trainingClassInfoBean : trainingClassInfoList) {
				int tempCount = 1;
				int tempAuth = 0;
				if (trainingClassInfoBean.getAcceptStatus().equals(TrainingClassInfoConfig.ACCEPT_STATUS_APPROVED)) {
					tempAuth = 1;
				}
